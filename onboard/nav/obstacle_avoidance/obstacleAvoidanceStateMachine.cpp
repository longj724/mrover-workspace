...
{
    double distanceToBall = phoebe->roverStatus().target().distance;
    double bearingToBall = phoebe->roverStatus().target().bearing;
    ...
    return distanceToObstacle > distanceToBall - targetThreshold ||
